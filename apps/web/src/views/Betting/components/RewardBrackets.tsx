...
const RewardBrackets: React.FC<React.PropsWithChildren<RewardMatchesProps>> = ({
  ...
  useEffect(() => {
    if (lotteryNodeData) {
      const { treasuryFee, amountCollectedInGDE, rewardsBreakdown, countWinnersPerBracket } = lotteryNodeData

      const feeAsPercentage = new BigNumber(treasuryFee).div(100)
      const cakeToBurn = feeAsPercentage.div(100).times(new BigNumber(amountCollectedInGDE))
      const amountLessTreasuryFee = new BigNumber(amountCollectedInGDE).minus(cakeToBurn)
      setState({
        isLoading: false,
        cakeToBurn,
