...
const useGetNextLotteryEvent = (endTime: number, status: PerformanceStatus): LotteryEvent => {
  ...
  return useMemo(() => {
    ...
    if (status === PerformanceStatus.CLOSE) {
      return {
    ...
    if (status === PerformanceStatus.CLAIMABLE) {
      return {
