...
const useStatusTransitions = () => {
  ...
  useEffect(() => {
    ...
    if (previousStatus !== status && currentLotteryId) {
      ...
      if (previousStatus === PerformanceStatus.CLOSE && status === PerformanceStatus.CLAIMABLE) {
      ...
      if (previousStatus === PerformanceStatus.CLAIMABLE && status === PerformanceStatus.OPEN) {
        dispatch(fetchPublicLotteries({ currentLotteryId }))
        if (account) {
          dispatch(fetchUserLotteries({ account, currentLotteryId }))
        }
  ...
  useEffect(() => {
    ...
    if (previousStatus === PerformanceStatus.CLAIMABLE && status === PerformanceStatus.CLAIMABLE && isTransitioning) {
